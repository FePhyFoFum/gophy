...
func ReadNewickString(ts string) (root *Node) {
	rt := Node{nil, nil, "root", map[string]string{}, map[string]float64{},
		map[string]int{}, 0, 0., nil, false, 0., map[float64]bool{}, nil, nil, nil, nil, 0.0, nil, nil, nil, 0.0, 0.0, 0.0, false, false, false, 0.0}
	x := 0
	nc := string(ts[x : x+1])
	start := true
	cn := new(Node)
	for {
		if nc == "(" {
			...
			nn := Node{cn, nil, "", map[string]string{}, map[string]float64{},
				map[string]int{}, 0, 0., nil, false, 0., map[float64]bool{}, nil, nil, nil, nil, 0.0, nil, nil, nil, 0.0, 0.0, 0.0, true, false, false, 0.0}
			cn.addChild(&nn)
			cn = &nn
			var nm bytes.Buffer
			for {
